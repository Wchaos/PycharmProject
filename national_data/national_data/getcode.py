...
def parse(content, level, parent_code):
    ...
    for index, node_content in enumerate(node_list, 1):
        ...
        code = "%s%s%s" % (parent_code, order_list[int(index / 36)], order_list[int(index % 36)]) #算出当前节点code
        # print(code)
...
def get_zhibiao_list():
    """获取包含所有节点的列表，列表元素为包含单个节点名字、code以及是否有子节点三个信息的字典"""
    if not os.path.exists('page_source.html'): #如果不存在文件，则获取子节点都打开后的html文件
        get_page()

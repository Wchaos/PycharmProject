...
class MongoDBPipeline(object):
    def __init__(self):
        ...
        db = clinet["Sinaweibo"]
        self.Information = db["Information_new"]
        self.Tweets = db["Tweets_new"]
        self.Relationships = db["Relationships_new"]

    def process_item(self, item, spider):
        """ 判断item的类型，并作相应的处理，再入数据库 """
        print("===============insert database mongodb======================")
        if isinstance(item, RelationshipsItem):
            print("---------------RelationshipsItem---------------------")
            print(RelationshipsItem)
            try:

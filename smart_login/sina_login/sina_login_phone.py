import json

import requests
import random

session = requests.session()
user_agents = [
    'Mozilla/5.0 (iPhone; CPU iPhone OS 9_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 '
    'Mobile/13B143 Safari/601.1]',
    'Mozilla/5.0 (Linux; Android 5.0; SM-G900P Build/LRX21T) AppleWebKit/537.36 (KHTML, like Gecko) '
    'Chrome/48.0.2564.23 Mobile Safari/537.36',
    'Mozilla/5.0 (Linux; Android 5.1.1; Nexus 6 Build/LYZ28E) AppleWebKit/537.36 (KHTML, like Gecko) '
    'Chrome/48.0.2564.23 Mobile Safari/537.36']

headers = {
    'User-Agent': random.choice(user_agents),
    'Referer': 'https://passport.weibo.cn/signin/login?entry=mweibo&res=wel&wm=3349&r=http%3A%2F%2Fm.weibo.cn%2F',
    'Origin': 'https://passport.weibo.cn',
    'Host': 'passport.weibo.cn'
}
headers2 = {
    'User-Agent': random.choice(user_agents),

}
post_data = {
    'username': '',
    'password': '',
    'savestate': '1',
    'ec': '0',
    'pagerefer': 'https://passport.weibo.cn/signin/welcome?entry=mweibo&r=http%3A%2F%2Fm.weibo.cn%2F&wm=3349&vt=4',
    'entry': 'mweibo'
}
# 这个入口仍然有效
login_url = 'https://passport.weibo.cn/sso/login'


def login():
    # username = input('请输入用户名:\n')
    # password = input('请输入密码：\n')
    # post_data['username'] = username
    # post_data['password'] = password
    post_data['username'] = "17084633974"
    post_data['password'] = "haha123456"
    print(login_url)
    r = session.post(login_url, data=post_data, headers=headers)
    if r.status_code != 200:
        raise Exception('模拟登陆失败')
    else:
        print("模拟登陆成功,当前登陆账号为：" + post_data['username'])
        print(r.content.decode())
        print(eval(r.content.decode()))
        print(eval(r.content.decode())["data"]["crossdomainlist"])
        crossdomaindict = dict(eval(r.content.decode())["data"]["crossdomainlist"])
        for item in crossdomaindict.keys():
            print(crossdomaindict[item].replace("\/","/"))
            crossUrl =crossdomaindict[item].replace("\/","/")
            response = session.get(crossUrl,headers=headers2)
            print(response.content.decode("utf-8"))

        test_url = 'https://m.weibo.cn/u/5837697857?uid=5837697857'
        test_url1 = "https://weibo.cn/5235640836/profile?filter=1&page=1"
        test_url2 = "http://weibo.com/5235640836/profile?topnav=1&wvr=6&is_all=1"
        response = session.get(test_url2,headers=headers2)
        print(response.content.decode("utf-8"))


if __name__ == '__main__':
    login()

...
def getType(browser):
    ...
    im0.crop((int(box.location['x']) + 10, \
              ...
              int(box.location['y']) + box.size['height'] - 10)).save("bb.png")
    im = Image.open("bb.png").convert("L")
    newBox = getExactly(im)
    im.crop(newBox).save("cc.png")
    im = im.crop(newBox)
    width = im.size[0]
    height = im.size[1]
    ...
    for png in ims.keys():
        ...
        for i in range(width):
            for j in range(height):
                print(im.load()[i, j])
                distance += numpy.square(im.load()[i,j]-ims[png][i][j])
...
if __name__ == '__main__':
    ...
    login = browser.find_element_by_id('loginAction')
    name.send_keys('17077126433')  # 测试账号
    psw.send_keys('ddyqht69011')
    login.click()
    time.sleep(5)
